...
# ────────────────────────────── CONFIG (FIXED: 🇰🇪 Emoji for Flag) ──────────────────────────────
st.set_page_config(page_title="LearnFlow AI", page_icon="🇰🇪", layout="wide")  # ← FIXED: Actual Kenya flag emoji
...
def login_block():
    ...
    with st.container():
            ...
            if len(pwd) < 6:
                ...
                try:
                    ...
                    if not user:
                        ...
                        # DEBUG: Log hash check
                        hash_check = bcrypt.checkpw(pwd.encode('utf-8'), user["password_hash"].encode('utf-8'))
                        print(f"DEBUG LOGIN: Email={email}, Hash Check={hash_check}, Stored Hash={user['password_hash'][:10]}...")  # Truncated for log
                        
                        if not hash_check:
                            st.error("Invalid email or password")  # ← This is the "wrong credentials" message
        ...
        st.markdown("---")
        st.info("**Demo Login:** `kingmumo15@gmail.com` / `@Yoounruly10` (admin)")
...
def sidebar():
    with st.sidebar:
        ...
        st.markdown("### Badges")
        badges = json.loads(user.get("badges", "[]")) if user else []
        for b in badges[:5]:
