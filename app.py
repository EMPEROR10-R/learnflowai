...
def login_block():
    ...
    if st.button(choice, type="primary"):
        ...
        if not user or not bcrypt.checkpw(pwd.encode(), user["password_hash"].encode() if isinstance(user["password_hash"], str) else user["password_hash"]):
        ...
        st.session_state.update({"logged_in": True, "user_id": user["user_id"], "is_admin": user["role"] == "admin", "user": user})
        st.rerun()
