...
class Database:
    ...
    def init_db(self):
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS users (
            user_id TEXT PRIMARY KEY, name TEXT, email TEXT UNIQUE, password_hash TEXT,
            role TEXT DEFAULT 'user', is_premium INTEGER DEFAULT 0, two_fa_secret TEXT, parent_id TEXT
        )''')
        c.execute('''CREATE TABLE IF NOT EXISTS activity_log (
            id INTEGER PRIMARY KEY AUTOINCREMENT, user_id TEXT, action TEXT, timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        )''')
        c.execute('''CREATE TABLE IF NOT EXISTS manual_payments (
            id INTEGER PRIMARY KEY AUTOINCREMENT, user_id TEXT, phone TEXT, mpesa_code TEXT,
            status TEXT DEFAULT 'pending', requested_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, processed_at TIMESTAMP
        )''')
        c.execute('''CREATE TABLE IF NOT EXISTS quiz_scores (
            id INTEGER PRIMARY KEY AUTOINCREMENT, user_id TEXT, subject TEXT, score INTEGER, total INTEGER, timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        )''')
        c.execute('''CREATE TABLE IF NOT EXISTS user_badges (
            id INTEGER PRIMARY KEY AUTOINCREMENT, user_id TEXT, badge_key TEXT, unlocked_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    def get_monthly_revenue(self) -> Dict:
        ...
        c = conn.cursor()
        c.execute("""
            SELECT strftime('%Y-%m', processed_at) AS month, COUNT(*) * 500 AS revenue
            FROM manual_payments WHERE status='approved' AND processed_at IS NOT NULL
    ...
    def get_leaderboard(self, limit: int = 10) -> List[Dict]:
        ...
        c.execute("""
            ...
            GROUP BY q.user_id ORDER BY total_score DESC LIMIT ?
        """, (limit,))
        rows = c.fetchall()
        conn.close()
        return [dict(r) for r in rows]
